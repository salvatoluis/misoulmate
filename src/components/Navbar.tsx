      ...
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-6">
          {activeLinks.map((link, index) => (
            <Link
              key={link.id}
              to={link.href}
              className={`text-sm font-medium hover:text-emerald-500 transition-colors duration-200 relative group ${
                scrolled ? "text-[#2B2B2A]" : "text-white"
              }`}
              style={{
                animation: `fadeInDown 0.5s ease forwards`,
          ...
          {isAuthenticated ? (
            ...
            <div className="">
              <button
                className={`font-medium px-4 py-2 rounded-full transition-all duration-200 hover:scale-105 active:scale-95 ${
                  scrolled
                    ? "text-[#2B2B2A] bg-gray-100"
        ...
        >
          {mobileMenuOpen ? (
            ...
            <Menu
              size={24}
              className={scrolled ? "text-[#2B2B2A]" : " text-white"}
            />
          )}
        </button>
      </div>
